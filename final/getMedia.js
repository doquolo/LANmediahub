  ...
  function createImgFrame(list) {
    ...
    for (let i in list) {
      if (list[i][2] == "image") {
        ...
        let elem = `
            <video id="vid" width="300" height="300" preload="none" onclick="expandImage(this)" src="/image?image=${list[i][0]}&folder=${list[i][1]}" paused muted poster="play.png"></video><br/>
            `;
        container.innerHTML += elem;
      }
    }
